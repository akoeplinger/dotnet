...
if [ -z "$IGNOREFlavor" ] || [ "$IGNOREFlavor" != "Product" ]; then
  export NUGET_PACKAGES=$scriptroot/../../artifacts/source-build/self/package-cache/
fi

error "Foo"

#"$DOTNET" msbuild "$scriptroot/source-build.proj" /p:Configuration=$configuration /p:IGNORE=true /p:ArcadeBuildFromSource=true "/p:RepoRoot=$scriptroot/../../" "/bl:$scriptroot/../../artifacts/source-build/self/log/source-build.binlog" $args
